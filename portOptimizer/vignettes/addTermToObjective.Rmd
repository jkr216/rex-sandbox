...
In this example we want the following terms in the objective definition:     
...
# The portfolio functions    
These functions are included in the package.  User defined / custom functions may be used as well. The first parameter should be x, a vector of portfolio weights.    
* calc_port_return     
* calc_port_risk    
* calc_port_TE    
* calc_nnonzeroassets    
...
The following code loads and creates some data which will be used in the example
```{r createData}
...
# definition of some groups.  We use these to control the allocation to groups of assets (group constraints).
...
The function `addTermToObjective` creates a list which defines a function, but does not actually create a function.  
The `calc_objective` function takes x and an objective definition and returns the value of the objective. Thus `calc_objective` can be the objective function if you don't want to build one.
